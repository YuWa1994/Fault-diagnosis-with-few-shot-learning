...
# ---------------------------------------------
#  transfer learning with matching network
# ---------------------------------------------
...
### Define Nets
embedding = MN.embedding(in_size=trainX.shape[1])

### Train source task
model = MN.Matching_net(trainX, trainY, n_epoch=200, is_normalization=True)
model = model.train(embedding)
     
### train target task
trainX, trainY, testX, testY = get_data(datapath+target_data, is_fft=True)
trainX, trainY = get_sample_data(trainX, trainY, num=5, classes=10) # randomly select a small number of training data    
model = model.transfer(trainX, trainY, n_epoch=20)
...
sio.savemat(f'{save_result}/result-{model.n_support}shot-trans.mat', result)   
